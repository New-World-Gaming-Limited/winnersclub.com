#!/usr/bin/env python3
"""
Hindi (HI) translation script for 1win.codes - v3
Key fixes from v2:
- Protect "1win" from "win" translation (use word boundary)
- Protect plurals (deposits → "जमाs" bug fixed)
- All PHRASE replacements come BEFORE word replacements
- Word replacements use \bword\b regex pattern
"""
...
def make_hreflang_for_page(rel_path):
    ...
    lines = []
    for loc in LOCALES:
        url = f"https://1win.codes/{loc}/{slug}" if slug else f"https://1win.codes/{loc}/"
...
# TRANSLATION SYSTEM
# Two tiers:
# 1. PHRASE_REPLACEMENTS: exact multi-word replacements (applied first, no word boundary)
# 2. WORD_REPLACEMENTS: single-word replacements with word boundary protection
#
# PROTECTED STRINGS (never translate):
# 1win, XLBONUS, Curaçao 8048/JAZ, all brand/game/payment names
# =============================================================================

# Tier 1: Multi-word / phrase replacements (longest first, exact match)
PHRASE_REPLACEMENTS = [
    # Navigation exact phrases
    ...
    ("This is an unofficial fan site and is not affiliated with or endorsed by 1win. This site is for informational purposes only. Gambling involves risk. Please play responsibly. If you or someone you know has a gambling problem, please contact GamCare or a local support organization.",
     "यह एक अनौपचारिक फैन साइट है और 1win से संबद्ध या अनुमोदित नहीं है। यह साइट केवल सूचनात्मक उद्देश्यों के लिए है। जुए में जोखिम शामिल है। कृपया जिम्मेदारी से खेलें। यदि आप या आपके परिचित को जुए की समस्या है, तो GamCare या स्थानीय सहायता संगठन से संपर्क करें।"),
    ...
    # Body content - long phrases (most specific first)
    ...
    ("1win is global. This website can be accessed all over the world. Log in to your account to start playing casino games and placing sports bets. Log in on your PC or mobile device to start playing. Alternatively, download the official 1win app to play on your Android or iOS device.",
     "1win वैश्विक है। इस वेबसाइट को पूरी दुनिया में एक्सेस किया जा सकता है। कैसीनो गेम्स खेलने और स्पोर्ट्स बेट लगाने के लिए अपने खाते में लॉगिन करें। PC या मोबाइल पर लॉगिन करें और खेलना शुरू करें। Android या iOS पर खेलने के लिए आधिकारिक 1win ऐप डाउनलोड करें।"),
    ("When you log in you can play immediately. You have lots of payment methods to choose from and can fund your account using Bitcoin and many other crypto coins.",
     "जब आप लॉगिन करते हैं तो आप तुरंत खेल सकते हैं। आपके पास चुनने के लिए कई भुगतान विधियां हैं और Bitcoin तथा कई अन्य क्रिप्टो कॉइन्स से अपना खाता फंड कर सकते हैं।"),
    ...
    ("allows you to get the biggest available bonus when opening a new account. By using this code when registering, you can get a 600% deposit bonus. This can be claimed at either the sportsbook or the casino and also comes with up to 400 free spins.",
     "का उपयोग करके नया खाता खोलते समय सबसे बड़ा उपलब्ध बोनस प्राप्त करें। रजिस्ट्रेशन के समय इस कोड का उपयोग करके 600% जमा बोनस और 400 फ्री स्पिन तक पाएं।"),
    ...
    ("Make your first deposit. A 130% first deposit bonus is available with",
     "अपना पहला जमा करें। 130% पहला जमा बोनस"),
    ...
    # Country names (multi-word first)
    ("South Africa", "दक्षिण अफ्रीका"),
    ("Tamil Nadu", "तमिलनाडु"),
    ("Uttar Pradesh", "उत्तर प्रदेश"),
    ("West Bengal", "पश्चिम बंगाल"),
    ...
    ("Curaçao-licensed (8048/JAZ)", "Curaçao 8048/JAZ लाइसेंसधारी"),
    ("Curaçao-licensed", "Curaçao 8048/JAZ लाइसेंसधारी"),
    ("Curacao-licensed", "Curaçao 8048/JAZ लाइसेंसधारी"),
    ("Curaçao licence 8048/JAZ", "Curaçao 8048/JAZ लाइसेंस"),
    ("Curacao licence 8048/JAZ", "Curaçao 8048/JAZ लाइसेंस"),
    ("Curaçao licensed (8048/JAZ)", "Curaçao 8048/JAZ लाइसेंसधारी"),
]

# Sort phrases by length (longest first) for safe replacement
...
def translate_text(text):
    ...
    IMPORTANT: Does NOT translate 1win, XLBONUS, brand names.
    """
    if not text or not text.strip():
        return text
    
    # Apply phrase replacements (longest first, exact string match)
    for en, hi in PHRASE_REPLACEMENTS:
        if en in text:
            text = text.replace(en, hi)
    
    # Apply single-word navigation terms (these are safe UI terms)
    word_map = {
    ...
    for en, hi in word_map.items():
        # Use word boundary to avoid partial replacements
        # e.g., "Cricket" should not match inside "Cricketers"
        text = re.sub(r'\b' + re.escape(en) + r'\b', hi, text)
...
def fix_structural_elements(content, rel_path):
    ...
    # 3. Fix canonical URL /en/ → /hi/
    content = re.sub(
        r'<link rel="canonical" href="https://1win\.codes/en/([^"]*)"[^>]*>',
        lambda m: f'<link rel="canonical" href="https://1win.codes/hi/{m.group(1)}">',
        content
    )
    
    # 4. Fix og:url if present
    content = re.sub(
    ...
    # 5. Remove existing hreflang links (all variants)
    ...
    # 6. Fix navigation href="/en/..." → href="/hi/..."
    ...
    # 8. Normalize Curacao spelling
    content = content.replace('Curacao 8048/JAZ', 'Curaçao 8048/JAZ')
    content = re.sub(r'\bCuracao\b(?! 8048)', 'Curaçao', content)
...
def translate_meta_tags(content):
    ...
    # aria-label
    aria_map = {
        'aria-label="Sign Up"': 'aria-label="साइन अप करें"',
        'aria-label="Toggle menu"': 'aria-label="मेनू टॉगल करें"',
        'aria-label="Language"': 'aria-label="भाषा"',
        'aria-label="1win home"': 'aria-label="1win होम"',
        'aria-label="Breadcrumb"': 'aria-label="ब्रेडक्रम्ब"',
    }
...
def translate_html_body(content):
    """
    Translate visible text content in HTML body.
    Skips: script, style, code, pre, noscript blocks.
    Skips: inside HTML tags (attributes).
    Translates: text nodes between tags.
    """
    output = []
    i = 0
    n = len(content)
    ...
    while i < n:
        ...
        if skip_m:
            ...
            close_str = SKIP_CLOSE.get(tag_name, f'</{tag_name}')
            close_idx = remaining.lower().find(close_str.lower(), skip_m.end())
            if close_idx != -1:
                # Find the end of the closing tag
                close_end = remaining.find('>', close_idx)
                if close_end != -1:
                    close_end += 1
                else:
                    close_end = len(remaining)
                block = remaining[:close_end]
            else:
                block = remaining
                close_end = len(remaining)
            output.append(block)
...
def ensure_curacao_present(content):
    """Ensure Curaçao 8048/JAZ appears in the content."""
    ...
    if first_p:
        ...
        new_text = old_text.rstrip('.').rstrip() + '. 1win Curaçao 8048/JAZ लाइसेंस के तहत संचालित होता है।'
...
def audit_page(hi_content, rel_path):
    ...
    if '/hi/' not in hi_content:
        issues.append("missing /hi/ path")
    
    hreflang_count = hi_content.count('hreflang=')
    if hreflang_count < 40:
        issues.append(f"only {hreflang_count} hreflang entries (need 47+)")
    
    # Check "1win" not corrupted
    if '1जीत' in hi_content:
        count = hi_content.count('1जीत')
        issues.append(f"1win corrupted to 1जीत ({count} times)")
    
    # Check for plural suffix corruption
    if 'जमाs' in hi_content:
        issues.append("plural corruption: जमाs")
    if 'जीतning' in hi_content:
        issues.append("partial translation: जीतning")
    if 'बेटting' in hi_content:
        issues.append("partial translation: बेटting")
    ...
    for i, block in enumerate(jsonld_blocks):
        try:
            ...
            issues.append(f"JSON-LD block {i+1} invalid: {str(e)[:60]}")
...
def process_all():
    ...
    for idx, rel_path in enumerate(inventory):
        ...
        try:
            ...
            if (idx + 1) % 25 == 0:
                issue_count = len(results['audit_issues'])
                print(f"  [{idx+1}/{len(inventory)}] done, pages with issues: {issue_count}")
...
if __name__ == "__main__":
    ...
    if results['audit_issues']:
        print(f"\nAudit issues (first 30):")
        for path, issues in list(results['audit_issues'].items())[:30]:
            print(f"  {path}: {issues}")
